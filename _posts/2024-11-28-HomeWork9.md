...
Corrected Sample Variance:
  ...
  Variance: ${(correctedVariances.reduce((sum, v) => sum + Math.pow(v - correctedMean, 2), 0) / m).toFixed(4)}

Uncorrected Sample Variance:
  Mean: ${uncorrectedMean.toFixed(4)}
  Variance: ${(uncorrectedVariances.reduce((sum, v) => sum + Math.pow(v - uncorrectedMean, 2), 0) / m).toFixed(4)}
